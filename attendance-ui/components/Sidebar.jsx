  ...
  return (
    <aside className="w-64 h-full flex flex-col bg-glass-card backdrop-blur-xl border-r border-glass-border">
      ...
      <div className="px-6 py-6 flex items-center gap-4 border-b border-glass-border relative overflow-hidden">
        ...
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-blue to-brand-cyan flex items-center justify-center text-foreground font-bold shadow-glow-brand relative z-10">
          V
        </div>
        <div className="relative z-10">
          <div className="text-lg font-bold tracking-tight text-foreground">VisageAI</div>
      ...
      <div className="p-4 border-t border-glass-border mt-auto bg-white/[0.02] backdrop-blur-md">
        ...
        <AnimatePresence>
          {showLogoutConfirm && (
            ...
            >
              <p className="text-xs text-foreground/60 mb-2 text-center">Sign out of VisageAI?</p>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowLogoutConfirm(false)}
                  className="flex-1 py-1.5 rounded-lg text-xs text-foreground/50 hover:text-foreground bg-foreground/5 hover:bg-foreground/10 transition-all"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={loggingOut}
                  className="flex-1 py-1.5 rounded-lg text-xs text-red-400 hover:text-foreground bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 transition-all font-semibold"
                >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-shrink-0">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-blue/80 to-brand-cyan/80 flex items-center justify-center text-foreground font-bold text-xs border border-foreground/10 shadow-[0_0_10px_rgba(6,182,212,0.3)]">
              {initials}
            </div>
